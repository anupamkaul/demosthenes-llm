...
Extend causal attention class over multiple heads (stacked up causal self-attention units). This is multi-head attention.
...
there is only 1 set of attention weights processing the input sequentially. (Heads here refers to multiple units stacked on top of
each other. These are the same functional units, like Wt Matrices, Q/K/V matrices, and the Context Vector outputs.
...
context vectors that get generated. (Its like input is the same but everything else is directly stacked up in Z-order). 

E.g. if there were 2 Causal SA modules, we now have multi-headed attention with 2 modules. Two value weight matrices for ex. Wv1 and Wv2. 
Same applies to other Wt matrices, Wq and Wk.
We obtain 2 sets of context vectors Z1 and Z2 that we can finally combine into a single context vector matrix Z.
...
class MultiHeadAttentionWrapper(nn.Module):
    ...
    def __init__(self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=False):
        ...
        self.heads = nn.ModuleList(
            ...
            [SelfAttn.SelfAttentionV2Causal( d_in, d_out, context_length, dropout, qkv_bias)
