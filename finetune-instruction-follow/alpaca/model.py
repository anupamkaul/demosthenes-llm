...
For Alpaca dataset based instruction fine tuning of demosthenes: 
...
try:
    model.load_state_dict(torch.load("./model/modelif-alpaca.pth", map_location=device))
    print("loaded previously saved model to continue training for instruction-follow..<enter>")
    input()

except FileNotFoundError:
    print("model not found on disk. train from scratch for instruction-follow for alpaca dataset .. <enter>")
    input()
...
'''
What I get and need to optimize this:

Training loss: 3.3177586078643797
Validation loss: 3.503592777252197
