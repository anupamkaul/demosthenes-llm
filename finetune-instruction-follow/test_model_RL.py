...
for entry in test_data:    # iterate over all samples
    ...
    # explore if on a Bad or Retry option, if modifying the prompt
    # brings out better suggestions. This could be a framework for
    # RL with human loop to check/improve LLM performance

    # Right now it repeats the same response as the prompt is the same
    # Plus there is no built in RL feedback loop in demosthenes for IF
    ...
    while True:
        ...
        print(f"\nModel response:\n>> {response_text.strip()}")
        print("-------------------------------------")
        user_response = input("Good[G], Bad[B], Retry[R]: ") #let it flow
