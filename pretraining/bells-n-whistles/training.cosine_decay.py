...
def train_model_simple(model, train_loader, val_loader, optimizer, device, num_epochs,
    ...
    for epoch in range(num_epochs):                           # main training loop
        ...
        for input_batch, target_batch in train_loader:
            ...
            '''
            # no need for the actual training here
...
# use default lr of Adam which is 0.0001 (10^-3) 
# then lr warmup applies during the training loop
optimizer = torch.optim.AdamW(
    model.parameters(),
    weight_decay=0.1 
)
...
initial_lr = 0.0001 #10^-3

# we don't need peak_lr for cosine decay
# but here we will combine both lr warmup
# followed by cosine decay

peak_lr = 0.01      #10^-2

# for cosine decay since we go downward in 
#a half cosine cycle, we need min_lr

min_lr = 0.1 * initial_lr

# typically the number of warmup steps is between 0.1% to 20% of the total number of steps
# We can compute the increment as the difference between the peak_lr and initial_lr, divided
# by the number of warmup steps
...
print("lr_increment: ", lr_increment)
input("enter<>")
